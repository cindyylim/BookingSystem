...
public class UserController {
    ...
    public ResponseEntity<?> getUserAppointments(Authentication auth) {
        ...
        List<Appointment> upcoming = all.stream().filter(a -> a.getTimeSlot().getStartTime().isAfter(now))
                .collect(Collectors.toList());
        List<Appointment> history = all.stream().filter(a -> a.getTimeSlot().getStartTime().isBefore(now))
                .collect(Collectors.toList());
        return ResponseEntity.ok(Map.of(
                "upcoming", upcoming,
                "history", history));
